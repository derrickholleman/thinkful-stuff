...
import users from '../src/data.json';
import {createMemoryHistory} from 'history'
import '@testing-library/jest-dom/extend-expect';
import User from "../src/User";
import UserPosts from "../src/UserPosts";
...
describe('Nested routes', () => {
  describe('User component', () => {
    ...
    test('displays UserPosts when url ends with /posts', () => {
      ...
      expect(getByText('asperiores ea ipsam voluptatibus modi minima quia sint')).toBeTruthy()
    ...
    test('uses route URL in links', () => {
      ...
      expect(getByTestId('user-posts').getAttribute("href")).toBe("/spot/5/posts")
  ...
  describe('UserPosts component', () => {
    test('UserPosts component uses route URL in links', () => {
      ...
      expect(getByTestId('user-post-11').getAttribute("href")).toBe("/lake/2/posts/11")
      expect(getByTestId('user-post-20').getAttribute("href")).toBe("/lake/2/posts/20")
    ...
    test('displays UserPost when url ends with :postID', () => {
      ...
      expect(getByText('doloremque illum aliquid sunt')).toBeTruthy()
    })
  })
})
